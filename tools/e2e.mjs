...
const scenarios = {
  ...
  async telemetry(api) {
    ...
    ok(boot.enabled === false && boot.pending === 1 && boot.q[0].event === 'session_start' && boot.q[0].properties.channel === 'dev' && boot.q[0].properties.source === 'web' && boot.q[0].properties.host === '' && boot.q[0].properties.standalone === false && boot.q[0].properties.sessions === 1 && /^[0-9a-f]{16}$/.test(boot.device) && boot.q[0].distinct_id === boot.device, `a file:// open queues session_start without sending (device ${boot.device})`);
