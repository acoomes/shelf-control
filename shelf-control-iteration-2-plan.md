...
- `tools/curate.mjs`: for each art × preset, run the existing generator (64 candidates, rated, six finalists re-rated), keep the candidate closest to the band, and emit the baked `LEVELS` block into `index.html`. Regenerated, never hand-edited. Every baked level keeps passing the loader and the solver in the headless suite. A curated level's id carries a hash of its layout (`art-slot-hash`), because progress and telemetry are keyed by id: a re-bake that changes a puzzle must not inherit a "done" earned on a different one, and the loader rejects an id that does not match its lanes.
- Difficulty curve, measured not felt: order by random-win rate descending with greedy-solvable levels first. Chapter 1 is the onboarding chapter and has no boss: eight levels in is too soon to ask a stranger to plan, and the tutorial's whole job is to get them to chapter 2. The first "needs lookahead" level closes chapter 2 at level 16; from chapter 3 on, one hard (≤ 5 %) level per eight, each a chapter's last level. So the bosses sit at levels 16, 24, 32 and 40 and nowhere else; the curation tool refuses a curve shaped otherwise and the headless suite asserts the baked set matches. (Baked 2026-09-24: the level-16 boss came out at 0.07 rather than the 0.12 aimed for, because no greedy-losing candidate sat nearer; it stays, on the argument that lookahead levels are about thinking, not luck, and the "+1 box" continue is there.) Level 1 stays as it is (the tutorial is tuned to it); the other five references keep their hand-made lanes and take the slots their measured rating earns, so the curve never runs backwards to fit them. Curation also enforces the prototype's H4 band as written there: 25–40 dispatches per level, so a level runs 45–90 s. If the art set cannot fill 40 slots inside that band, the band is widened by a written decision that re-opens H4, never by the tool.
- Level select scrolls in pages of eight; done/not-done plus best time, which the game already stores. No stars, no currencies.

Acceptance: 40 levels live; bosses at exactly levels 16, 24, 32 and 40, the last three hard, chapter 1 without one; the evaluator's curve is monotone within ±0.10 inside each chapter (the boss that closes it is the deliberate exception); every level solvable; every curated level's reference line fits the dispatch band. The six references are exempt from the band because their lanes are frozen for the Appendix B cross-check: heart runs 23 as the short tutorial and rainbow 42 as the finale, and the headless suite names exactly those two, so a third would fail it. The curation tool refuses to bake a slot with no in-band candidate; `--allow-off-band` is the written decision that overrides it.
