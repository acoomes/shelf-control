...
- **Hard preset.** *Hard* is a ceiling (≤ 0.05) that is also meant to force lookahead, so a candidate the greedy player loses and that sits within the ±0.10 acceptance band outranks one the greedy player solves; inside the ceiling the tie-break is greedy-loses, then the lower rate. If the winner is still greedy-solvable, one extra batch of 32 candidates is tried. Some arts cannot get there at all (Heart, Chick), and the tile then shows the achieved rating and "greedy wins" honestly.
