...
- **Listed on itch.io** on 2026-09-25 at https://ajcdrift.itch.io/shelf-control, the portal for the stranger playtest. The upload is the live build stamped `source: itch`, so its players are the retention gate's cohort; from the next ship on, the `itch.io build` workflow pushes each ship there with butler.

The rest of slice 3 is people work: the stranger playtest itself, and H1 and H3 answered in writing.
