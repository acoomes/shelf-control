...
- **Telemetry to PostHog.** Five events over plain HTTP: `session_start`, `level_start`, `level_end` (the play record the game already kept), `daily_share` and `install`. No PostHog script on the page, no cookies, an anonymous device id as the only identity, and a `channel` property that tells the live build from the test build in one project. Events queue in local storage and flush with keepalive fetches, so a session played offline in the installed app reports when the device is next online. *Anonymous play stats* in settings turns it off. The debug overlay shows what was sent and what is pending.
